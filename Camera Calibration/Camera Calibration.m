...
[Rc,Tc,Kc,perf,perfSSE]=cameraCalibration(x,X,R,T,K);
...
function [Rc,Tc,Kc,perf,perfSSE]= cameraCalibration(x,X,R,T,K)
A= [ x(:,1).* X(:,1)   x(:,1).* X(:,2)   x(:,1).* X(:,3)   x(:,1)   -x(:,2).* X(:,1)   -x(:,2).* X(:,2)   -x(:,2).* X(:,3)  -x(:,2) ];
[U D V]=svd(A);
atemp=V(:,end);
r2temp=[atemp(1) atemp(2) atemp(3)];
gamma=sqrt(r2temp*r2temp');
ac=atemp/gamma;
r2=[ac(1) ac(2) ac(3)];
r1temp=[ac(5) ac(6) ac(7)];
alphaa=sqrt(r1temp*r1temp');
r1=r1temp/alphaa;

Ty=ac(4);
Tx=ac(8)/alphaa;

if (x(2)*(r2(1)*X(1) + r2(2)*X(2) + r2(3)*X(3)) < 0)
    r1(1) = -1*r1(1);
    r1(2) = -1*r1(2);
    r1(3) = -1*r1(3);
    r2(1) = -1*r2(1);
...
a=  [x(:,1) r1(1).*X(:,1)+r1(2).*X(:,2)+r1(3).*X(:,3)+Tx]
b = (-x(:,1).*(r3(1).*X(:,1)+r3(2).*X(:,2)+r3(3).*X(:,3)))
m=a\b;
Tz=m(1);
fx=-m(2);

Rc=[r1;r2;r3];
Kc=[fx 0 0;0 fx 0;0 0 1];
Tc=[Tx; Ty; Tz];

Rerr= R-Rc;
Terr= T-Tc;
Kerr= K-Kc;
perf=[mae(Rerr) mae(Terr) mae(Kerr)];
perfSSE= [sum(sum(Rerr.^2)) sum(sum(Terr.^2)) sum(sum(Kerr.^2)) ];
end
